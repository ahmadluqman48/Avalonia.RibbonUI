...
namespace AvaloniaUI.Ribbon.Controls
{
    ...
    [TemplatePart("PART_CollapseRibbonControl", typeof(MenuItem))]
    public class RibbonControl : TabControl, IRibbon
    {
        #region Static Properties

        public static readonly StyledProperty<IBrush> HeaderBackgroundProperty = AvaloniaProperty.Register<RibbonControl, IBrush>(nameof(HeaderBackground));
        public static readonly StyledProperty<IBrush> HeaderForegroundProperty = AvaloniaProperty.Register<RibbonControl, IBrush>(nameof(HeaderForeground));

        public static readonly StyledProperty<object> HelpPaneContentProperty =
            AvaloniaProperty.Register<RibbonControl, object>(nameof(HelpPaneContent));

        public static readonly StyledProperty<bool> IsCollapsedPopupOpenProperty = AvaloniaProperty.Register<RibbonControl, bool>(nameof(IsCollapsedPopupOpen));
        public static readonly StyledProperty<bool> IsCollapsedProperty = AvaloniaProperty.Register<RibbonControl, bool>(nameof(IsCollapsed));
        public static readonly StyledProperty<bool> IsMenuOpenProperty = AvaloniaProperty.Register<RibbonControl, bool>(nameof(IsMenuOpen));
        ...
        public static readonly RoutedEvent<RoutedEventArgs> MenuClosedEvent = RoutedEvent.Register<RibbonControl, RoutedEventArgs>(nameof(MenuClosed), RoutingStrategies.Bubble);
        public static readonly StyledProperty<IRibbonMenu> MenuProperty = AvaloniaProperty.Register<RibbonControl, IRibbonMenu>(nameof(Menu));
        public static readonly StyledProperty<Orientation> OrientationProperty = StackPanel.OrientationProperty.AddOwner<RibbonControl>();

        public static readonly RoutedEvent<RoutedEventArgs> RibbonControlKeyTipsOpenedEvent = RoutedEvent.Register<MenuBase, RoutedEventArgs>("RibbonControlKeyTipsOpened", RoutingStrategies.Bubble);

        public static readonly DirectProperty<RibbonControl, ObservableCollection<RibbonGroupBox>> SelectedGroupsProperty = AvaloniaProperty.RegisterDirect<RibbonControl, ObservableCollection<RibbonGroupBox>>(nameof(SelectedGroups), o => o.SelectedGroups, (o, v) => o.SelectedGroups = v);

        public static readonly DirectProperty<RibbonControl, ObservableCollection<Control>> TabsProperty = AvaloniaProperty.RegisterDirect<RibbonControl, ObservableCollection<Control>>(nameof(Tabs), o => o.Tabs, (o, v) => o.Tabs = v);
        ...
        {
            OrientationProperty.OverrideDefaultValue<RibbonControl>(Orientation.Horizontal);

            SelectedIndexProperty.Changed.AddClassHandler<RibbonControl>((x, e) => x.RefreshSelectedGroups());

            IsCollapsedProperty.Changed.AddClassHandler<RibbonControl, bool>(((sender, args) => sender.UpdatePresenterLocation(args.NewValue.Value)));

            KeyTip.ShowChildKeyTipKeysProperty.Changed.AddClassHandler<RibbonControl>(new Action<RibbonControl, AvaloniaPropertyChangedEventArgs>((sender, args) =>
            {
            ...
            TabsProperty.Changed.AddClassHandler<RibbonControl>((sender, e) => sender.RefreshTabs());
        }
        ...
        protected override Type StyleKeyOverride => typeof(RibbonControl);

        #region Methods

        public void ActivateKeyTips(IRibbon RibbonControl, IKeyTipHandler prev)
        {
        ...
        public void CycleTabs(bool forward)
        {
            ...
            //var tabs = ((AvaloniaList<object>)Items).OfType<RibbonControlTab>().Where(x => x.IsEffectivelyVisible && x.IsEnabled);
            ...
            {
                step();
                RibbonControlTab newSel = (RibbonControlTab)(((AvaloniaList<object>)Items).ElementAt(newIndex));
                bool contextualVisible = true;
                if (newSel.IsContextual)
                    contextualVisible = (newSel.Parent as RibbonControlContextualTabGroup).IsVisible;
        ...
        public void Open()
        {
            ...
            RaiseEvent(new RoutedEventArgs
            {
                RoutedEvent = RibbonControlKeyTipsOpenedEvent,
                Source = this,
            });
        }
        ...
        protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
        {
            ...
            var collapseRibbonControl = e.NameScope.Find<MenuItem>("PART_CollapseRibbonControl");
            if (collapseRibbonControl is { })
            {
                collapseRibbonControl.Click += (_, _) =>
                {
                    if (IsCollapsed)
                        IsCollapsedPopupOpen = false;
        ...
        protected override void OnPointerWheelChanged(PointerWheelEventArgs e)
        {
            ...
            if (ItemCount > 1)
            {
                if (((Orientation == Orientation.Horizontal) && (e.Delta.Y > 0)) || ((Orientation == Orientation.Vertical) && (e.Delta.Y < 0)))
                {
                    ...
                    {
                        newIndex--;
                        var newTab = Items.OfType<RibbonControlTab>().ElementAt(newIndex);
